...
func TestPartialSuccessWithNonNilPerms(t *testing.T) {
	...
	serverConf := &ServerConfig{
		PublicKeyCallback: func(conn ConnMetadata, key PublicKey) (*Permissions, error) {
			// Return non-nil Permissions along with a PartialSuccessError.
			// Since permissions must be nil when returning PartialSuccessError,
			// the server is expected to reject the connection.
